    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          setFormErrorMessage(
            "강의 썸네일을 불러오지 못 했습니다! 파일 형식을 확인해주세요"
          );
        }
      })
      .catch((err) => {
        ...
        return (
          ...
          >
            <div style={{ textAlign: "center", marginBottom: "2rme" }}>
              <Title level={2}>강의 등록하기</Title>
            </div>
            <h4 style={{ color: 'darkgrey' }}>
            강의 등록은 자유롭지만 부적절한 내용의 강의라고 판단될 시 통보없이 삭제될 수 있습니다
    				</h4>
            <br />
            <div>
              <Form style={{ minWidth: "475px" }} onSubmit={handleSubmit}>
                <Form.Item required label="강의 썸네일">
                  <div
                ...
                <Form.Item required label="강의 설명">
                  <Input
                    id="description"
                    placeholder="강의에 대한 설명을 간략히 적어주세요"
                    type="text"
                ...
                >
                  <Input
                    id="capacity"
                    placeholder="강의의 정원을 입력해 주세요(숫자만 입력할 수 있습니다)"
                    type="number"
