...
import { REGISTER_STUDYGROUP, EDIT_STUDYGROUP } from "./types";
import { STUDYGROUP_SERVER } from "../components/Config.js";

export function registerStudyGroup(dataToSubmit) {
  const request = axios
    .post(`${STUDYGROUP_SERVER}/uploadStudyGroup`, dataToSubmit)
...
export function editStudyGroup(dataToSubmit) {
  const request = axios
    .post(`${STUDYGROUP_SERVER}/editStudyGroup`, dataToSubmit)
    .then((response) => response.data);
